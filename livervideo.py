...
# Hàm để gửi khung hình qua WebSocket
def send_frame(client, server):
    ...
    while True:
        ...
        # Mã hóa khung hình để gửi qua WebSocket
        _, buffer = cv2.imencode('.jpg', frame_bgr)
        frame_string = base64.b64encode(buffer).decode('utf-8')

        # Gửi khung hình tới tất cả client qua WebSocket
        server.send_message_to_all(frame_string)
        print("Đã gửi khung hình tới client")

# Thiết lập WebSocket server
try:
    print("Server WebSocket đang khởi động...")
    server = WebsocketServer(host='0.0.0.0', port=8000)
    server.set_fn_new_client(send_frame)
    print("Server WebSocket đã khởi động trên cổng 8000")

    # Chạy server WebSocket
    server.run_forever()

except Exception as e:
    print(f"Lỗi khi khởi động WebSocket server: {e}")
    GPIO.cleanup()
